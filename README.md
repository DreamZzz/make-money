...
这套系统不是“保证赚钱”的黑箱荐股器，而是一套本地化的纪律执行工具。**现阶段以指数为核心**：用市场状态引擎判断阶段/热度/估值，给 T+1 权益仓位建议并按相对强弱搭配指数；主动个股选股因实测未跑赢指数（Alpha158 选股 IC 接近 0），暂作研究/shadow，等市场风格翻转再观察（详见 `/market` 与 `/tournament`）。合理目标是长期跟住核心指数、控制回撤，单年最大回撤仍可能达到 20-25%。首次使用建议先纸盘跑通，再小额跟随，不要一次性买满。
